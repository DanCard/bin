...
MODEL_PATH = os.path.expanduser(
    "~/llms/qwen3/coder-next-no-bf16/Qwen3-Coder-Next-Q8_0-00001-of-00004.gguf"
)
...
def generate_summary(transcript_path, output_path, max_tokens, url):
    ...
    try:
        ...
        create_google_doc(output_path)

    except requests.exceptions.RequestException as e:
        print(f"\n❌ Error calling llama-server: {e}")


def create_google_doc(file_path):
    """Uses the Gemini CLI to create a Google Doc from the markdown file."""
    ...
    try:
        ...
        # 2. Use gemini CLI to create the doc
        ...
        # Run gemini command without shell=True for safety
        subprocess.run(["gemini", prompt], check=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate structured meeting notes using local LLM."
    )
