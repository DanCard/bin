...
def setup_logging(audio_file):
    ...
    log_filename = f"{audio_file}_{timestamp}.canary.log"
    logging.basicConfig(
        level=logging.INFO,
...
def main():
    parser = argparse.ArgumentParser(description="Transcribe and Diarize audio using Canary-Qwen and Pyannote")
    ...
    try:
        ...
        # 2. Transcription
        logging.info("Loading Canary-Qwen-2.5B...")
        model = SALM.from_pretrained('nvidia/canary-qwen-2.5b', cache_dir='/home/dcar/llms/')
        ...
        # Save results
        output_txt = audio_file + ".canary.txt"
        output_json = audio_file + ".canary.json"
