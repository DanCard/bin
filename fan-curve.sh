...
do_curve() {
    ...
    log "  Curve: off → 1482RPM@45°C → 2557RPM@65°C → 3374RPM@80°C → 4030RPM@90°C → 4647RPM@95°C"
    log "  Hysteresis: rampdown 7°C below rampup for level 1, 10°C for levels 2-3"
    for fan in fan1 fan2; do
        log "$fan: writing rampup_curve=45,65,80,90,95"
        echo "45,65,80,90,95" | sudo tee "$SYSFS/$fan/rampup_curve" > /dev/null
        log "$fan: writing rampdown_curve=38,55,72,85,92"
        echo "38,55,72,85,92" | sudo tee "$SYSFS/$fan/rampdown_curve" > /dev/null
    ...
    log "  Curve: off → on@40°C → 60°C → 80°C → 90°C → 95°C"
    log "$fan: writing rampup_curve=40,60,80,90,95"
    echo "40,60,80,90,95" | sudo tee "$SYSFS/fan3/rampup_curve" > /dev/null
    log "fan3: writing rampdown_curve=33,50,72,85,92"
    echo "33,50,72,85,92" | sudo tee "$SYSFS/fan3/rampdown_curve" > /dev/null
