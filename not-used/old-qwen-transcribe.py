...
def print_startup_overview(audio_file, args):
    ...
    overview_lines = [
        ...
        "  3. Builds diarization-aligned transcription chunks",
        "  4. Transcribes chunks with the speech model",
        "  5. Assigns speaker labels to each transcribed segment",
        "  6. Writes final outputs to text and JSON",
        f"Max chunk size: {args.chunk_size}s",
...
def build_transcription_chunks(total_duration, diarization, max_chunk_size):
    """Create chunk boundaries at diarization turn edges, then split by max size."""
    boundaries = {0.0, max(0.0, float(total_duration))}
    for entry in diarization:
        start = max(0.0, min(float(total_duration), float(entry["start"])))
        end = max(0.0, min(float(total_duration), float(entry["end"])))
        if end > start:
    ...
    for idx in range(len(sorted_bounds) - 1):
        interval_start = sorted_bounds[idx]
        interval_end = sorted_bounds[idx + 1]
        if interval_end <= interval_start:
            continue

        current_start = interval_start
        while current_start < interval_end:
            current_end = min(current_start + max_chunk_size, interval_end)
            speaker = get_speaker_for_chunk(current_start, current_end, diarization)
            chunks.append({"start": current_start, "end": current_end, "speaker": speaker})
...
def main():
    ...
    parser.add_argument("--chunk_size", type=int, default=8, help="Max chunk size in seconds (diarization-aligned)")
    ...
    try:
        ...
        transcription_chunks = build_transcription_chunks(total_duration, diarization_data, args.chunk_size)
        final_output = []
        text_lines = []

        logging.info(
            f"Transcribing {len(transcription_chunks)} diarization-aligned chunks with batch_size={args.batch_size}..."
        )
        ...
        for i in tqdm(range(0, len(transcription_chunks), args.batch_size), desc="Transcribing"):
            ...
            for idx, ids in enumerate(output_ids):
                ...
                if text:
                    final_output.append(
                        {"start": meta["start"], "end": meta["end"], "speaker": meta["speaker"], "text": text}
                    )
