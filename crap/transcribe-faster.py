...
labeled by name once cosine similarity clears --enroll-threshold (default 0.75).
Matching is greedy one-to-one, then a second pass folds any still-unnamed speaker
into an already-assigned name if it clears --rejoin-threshold (default 0.80). That
second pass exists because diarization tends to split the person who talks most
into two speakers; without it only one fragment can hold the name and the other --
often the longer one -- stays SPEAKER_xx (see match_speakers).
...
# --------------------------------------------------------------------------- #
# Two-channel recordings (realtime's --record --mic: ch0 remote, ch1 local mic)
# --------------------------------------------------------------------------- #
def probe_channels(path):
    """Channel count of the first audio stream, or 0 if it can't be determined."""
    try:
...
def extract_channel(input_path, ch, output_dir, tag):
    """Decode a single channel of a multi-channel recording to 16 kHz mono."""
    base = os.path.basename(input_path)
    out = os.path.join(output_dir, f"{base}.{FILE_TAG}.{tag}.tmp.wav")
    logging.info(f"Extracting channel {ch} ({tag}) to 16kHz mono WAV: {out}")
    ...
    try:
        ...
        logging.error(f"FFmpeg channel extraction failed: {e.stderr.decode()}")
        sys.exit(1)
    return out

def rms_envelope(wav_path, hop_s):
    """Short-term RMS of a mono 16 kHz wav, one value per hop, in s16 units."""
    ...
    if n == 0:
        return np.zeros(0)
    x = wav[:n * hop].reshape(n, hop).astype(np.float64)
    return np.sqrt((x ** 2).mean(axis=1)) * 32768.0

def _runs(mask):
    """Yield (start_idx, end_idx) half-open runs of True in a boolean array."""
    if not len(mask):
        return
...
def local_speech_intervals(local_wav, remote_wav, margin, floor,
                           hop_s=0.02, close_gap_s=0.30, min_dur_s=0.25):
    """Time ranges where the local mic clearly dominates the remote channel.

    Both channels hear both parties: the far end leaks into ch1 because your mic
    picks up your own speakers, and you leak into ch0 as the echo the far end
    sends back. So an absolute level test can't separate them -- only a ratio can.
    Your direct voice sits far above either bleed path (measured ~5:1 on this
    setup), hence requiring ch1 to beat ch0 by `margin` as well as clear `floor`.

    Runs are then closed over short gaps (so a normal inter-word pause doesn't
    fragment one utterance) and short bursts dropped (so a cough or a syllable of
    bleed doesn't become a turn).
    """
    l = rms_envelope(local_wav, hop_s)
    r = rms_envelope(remote_wav, hop_s)
    n = min(len(l), len(r))
    if n == 0:
        return []
    l, r = l[:n], r[:n]
    active = (l > floor) & (l > margin * np.maximum(r, 1.0))

    gap_hops = int(round(close_gap_s / hop_s))
    for s, e in list(_runs(~active)):
        if (e - s) <= gap_hops and s > 0 and e < n:
            active[s:e] = True

    out = []
    for s, e in _runs(active):
        if (e - s) * hop_s >= min_dur_s:
            out.append((s * hop_s, e * hop_s))
    return out

def apply_local_channel(turns, words, intervals, local_name):
    """Fold local-mic speech into the diarized timeline as a known speaker.

    The local channel is ground truth about *who*, so it overrides diarization
    rather than competing with it: local intervals become turns owned by
    `local_name`, and any diarized turn is clipped out of those ranges. ch0 words
    landing inside a local interval are echo of the local speaker returning from
    the far end -- dropping them is what stops the same sentence appearing twice
    under two names.

    Returns (turns, words), both filtered/extended.
    """
    if not intervals:
        return turns, words
    ...
    for s, e, spk in turns:
        ...
        for ls, le in intervals:
            nxt = []
            for ps, pe in pieces:
                if le <= ps or ls >= pe:      # disjoint
                    nxt.append((ps, pe))
                    continue
                if ps < ls:                    # keep the head
                    nxt.append((ps, ls))
                if pe > le:                    # keep the tail
                    nxt.append((le, pe))
            pieces = nxt
        kept.extend((ps, pe, spk) for ps, pe in pieces if pe - ps > 0.05)

    kept.extend((ls, le, local_name) for ls, le in intervals)
    kept.sort(key=lambda t: t[0])

    def in_local(ws, we):
        mid = (ws + we) / 2.0
        return any(ls <= mid < le for ls, le in intervals)

    return kept, [(t, ws, we) for (t, ws, we) in words if not in_local(ws, we)]
...
def match_speakers(centroids, refs, threshold, rejoin_threshold=None):
    """Cosine-match diarized speakers to enrolled names, in two passes.

    Pass 1 is greedy one-to-one: the best (speaker, name) pair wins and spends
    both. That is right when diarization got the clustering right, but it fails
    the person diarization over-splits -- typically whoever talks most, shares a
    screen, or changes audio setup mid-call. Their voice arrives as two global
    speakers, only one name is available, and whichever fragment happens to score
    higher takes it. The other fragment -- often the far longer one -- is locked
    out by `used_name` and prints as SPEAKER_xx despite matching well.

    Pass 2 repairs exactly that: a speaker left unmatched may take a name pass 1
    already handed out, if it clears the stricter `rejoin_threshold`. Restricting
    pass 2 to already-assigned names is what keeps it safe -- an unenrolled
    participant cannot claim a free name it merely half-resembles; a speaker can
    only be folded into someone already positively identified in this meeting.
    Pass None for strict one-to-one.

    Returns ({SPEAKER_xx: name}, {SPEAKER_xx: cosine}) for all matches; the
    scores let auto-enrollment harvest only confident matches."""
    ...
    if rejoin_threshold is None or not used_name:
        return mapping, scores
    # Pass 2. sims is still sorted descending, so the first surviving pair for a
    # speaker is its best taken-name candidate. A rejoined speaker always scores
    # below the pass-1 winner for that name (that pair sorted earlier), so harvest
    # keeps sourcing the name's clips from the pass-1 cluster, not a fragment.
    for sim, spk, name in sims:
        if sim < rejoin_threshold:
            break
        if spk in used_spk or name not in used_name:
            continue
        mapping[spk] = name
        scores[spk] = sim
        used_spk.add(spk)
        logging.info(f"Rejoined {spk} -> '{name}' (cosine {sim:.3f}, split "
                     f"fragment of an already-matched speaker)")
    return mapping, scores
...
def main():
    ...
    parser.add_argument("--rejoin-threshold", type=float, default=0.80,
                        help="Cosine at which a still-unnamed speaker is folded into a name "
                             "already assigned to another speaker, i.e. treated as a diarization "
                             "split of that same person (default 0.80, stricter than "
                             "--enroll-threshold). 0 disables, restoring strict one-to-one "
                             "matching where each name labels at most one speaker.")
    parser.add_argument("--enroll-threshold", type=float, default=0.75,
                        help="Minimum cosine similarity to label a diarized speaker with an enrolled "
                             "name (higher = stricter; unmatched speakers stay SPEAKER_xx). Default "
                             "0.75: correct matches land at 0.85-0.90, while everything observed in "
                             "the 0.45-0.70 band was a false positive, so a lower bar buys wrong "
                             "names rather than extra coverage.")
    parser.add_argument("--local-channel", type=int, default=1,
                        help="For 2-channel recordings from realtime --record "
                             "--mic: which channel is your own mic (default 1; "
                             "ch0 is the remote party). Ignored for mono input.")
    parser.add_argument("--no-local-channel", dest="local_channel",
                        action="store_const", const=-1,
                        help="Treat a stereo recording as a plain mixdown and "
                             "attribute every speaker by diarization instead.")
    parser.add_argument("--local-name", default="Dani",
                        help="Speaker name for the local mic channel (default "
                             "'Dani' -- the owner of this machine's mic).")
    parser.add_argument("--local-margin", type=float, default=2.0,
                        help="How far the local channel must exceed the remote "
                             "one to count as you speaking (default 2.0). Both "
                             "channels hear both parties, so this ratio -- not "
                             "absolute level -- is what separates them.")
    parser.add_argument("--local-floor", type=float, default=180.0,
                        help="RMS noise floor for the local channel (default 180, "
                             "matching realtime's --rms-floor).")
    ...
    # Two-channel recordings keep the parties physically separated, so work from
    # the channels directly instead of a mixdown. Mixing them down would overlay
    # the local speaker's direct voice on the echo of it coming back in ch0 --
    # the same words twice, a few hundred ms apart, which is worse for ASR than
    # either channel alone.
    n_ch = probe_channels(audio_file)
    local_ch = args.local_channel
    use_local = n_ch >= 2 and local_ch >= 0 and local_ch < n_ch
    local_file = None
    if use_local:
        remote_ch = 0 if local_ch != 0 else 1
        logging.info(f"{n_ch}-channel input: ch{remote_ch} = remote (diarized), "
                     f"ch{local_ch} = local mic -> '{args.local_name}'")
        working_file = extract_channel(audio_file, remote_ch, output_dir, "remote")
        local_file = extract_channel(audio_file, local_ch, output_dir, "local")
    else:
        if n_ch >= 2 and local_ch >= n_ch:
            logging.warning(f"--local-channel {local_ch} is out of range for a "
                            f"{n_ch}-channel file; falling back to a mixdown.")
    ...
    try:
        with prevent_sleep(f"Transcribing {os.path.basename(audio_file)}"):
            ...
            if not args.diar_only:
                ...
                if use_local:
                    # Cheap relative to ch0: the local channel is mostly silence,
                    # which the VAD filter skips.
                    logging.info("Transcribing local mic channel...")
                    local_words = transcribe_words(model, local_file, args)
                    logging.info(f"Local channel: {len(local_words)} words")
            ...
            mapping, scores = match_speakers(
                centroids, refs, args.enroll_threshold,
                rejoin_threshold=args.rejoin_threshold or None)
            ...
            # 2b-ii. Local mic channel — attribution by construction, not by
            # voiceprint. This is the only speaker we can identify with certainty,
            # so it is applied after (and overrides) enrollment matching.
            if use_local:
                intervals = local_speech_intervals(
                    local_file, working_file,
                    margin=args.local_margin, floor=args.local_floor)
                total = sum(e - s for s, e in intervals)
                logging.info(f"Local channel: {len(intervals)} speech interval(s), "
                             f"{format_time(total)} attributed to "
                             f"'{args.local_name}'")
                before = len(words)
                turns, words = apply_local_channel(
                    turns, words, intervals, args.local_name)
                if before != len(words):
                    logging.info(f"Dropped {before - len(words)} ch0 word(s) as "
                                 f"echo of the local speaker")
                words = sorted(words + local_words, key=lambda w: w[1])
        ...
        for tmp in (working_file, local_file):
